...
with ZipFile(PATH_FILE) as file_zip:
    with file_zip.open(NAME_FILE) as file:
        ...
        _df = _df.replace({"REF_AREA": DICT_COUNTRY})
        _df = _df.sort_values(['UNIT_MEASURE', 'TIME_PERIOD', 'REF_AREA']).reset_index(drop=True)
        _df['TIME_PERIOD']= pd.to_datetime(_df['TIME_PERIOD'])
        print(f"#" * MARK)
        print(f"\nDtypes:\n{_df.dtypes}\n")
...
index_pais = _df['REF_AREA'].unique()
print(f"\n{index_pais}\n" )
index_unit = _df['UNIT_MEASURE'].unique()
print(f"{index_unit}\n" )
...
# for pais  in index_pais:
#     for unit in index_unit:
#         _df [ (_df['REF_AREA'] == pais) & (_df['UNIT_MEASURE'] == unit) ].reset_index(drop=True)

#         # print(_series)
#         obj_dict["series_id"] = str(count + 1)+"//"+_df['REF_AREA'].unique()
#         obj_dict["points"] = _series
#         obj_dict["fields"] = {"country":_df['REF_AREA'].unique(), "concept":"Gas World Database", "units":_df['UNIT_MEASURE'].unique(), "source":"JODI" }
#         _obj.append(obj_dict)
